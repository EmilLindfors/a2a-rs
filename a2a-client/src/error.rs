...
pub enum ClientError {
    ...
    #[error("WebSocket client not configured. Actionable Suggestion: Ensure that a WebSocket interface is defined in preferred_transport or additional_interfaces on the agent card, or manually configure a WebSocket URL using WebA2AClientBuilder::ws_url.")]
    WebSocketNotConfigured,

    /// Transport auto-detection failed
    #[error("Failed to auto-detect available transports: {0}. Actionable Suggestion: Make sure the agent server is running, the host is reachable, and the `/agent-card` endpoint is accessible.")]
    AutoDetectionFailed(String),

    /// Invalid configuration
    #[error("Invalid configuration: {0}. Actionable Suggestion: Verify that builder configuration parameters, such as base URLs, are correctly specified.")]
    InvalidConfiguration(String),

    /// URL parsing/formatting failure
    #[error("Invalid URL format: '{url}'. Reason: {reason}. Actionable Suggestion: Double check that the URL scheme and address are valid and formatted correctly.")]
    InvalidUrl { url: String, reason: String },

    /// Serialization/deserialization error
    #[error("Data serialization error: {0}. Actionable Suggestion: Check that payloads adhere to the expected format and structure.")]
