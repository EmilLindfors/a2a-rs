...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let text_message = Message::builder()
        .role(Role::User)
        .parts(vec![Part::text("I need to get reimbursed $150.50 for client lunch at downtown restaurant".to_string())])
        .message_id(Uuid::new_v4().to_string())
        .context_id("conv-123".to_string())
        .build();

    let task_id = format!("task_{}", Uuid::new_v4().simple());
    let result = handler.process_message(&task_id, &text_message, None).await?;
    ...
    let task_id = format!("task_{}", Uuid::new_v4().simple());
    let result = handler.process_message(&task_id, &data_message, None).await?;
    println!("Response: {:?}\n", result.status.message);
    ...
    let task_id = format!("task_{}", Uuid::new_v4().simple());
    let result = handler.process_message(&task_id, &form_message, None).await?;
    println!("Response: {:?}\n", result.status.message);
    ...
    let mixed_message = Message::builder()
        ...
        .parts(vec![
            ...
            Part::File {
                file: file_content,
                metadata: json!({"extracted_amount": "$75.50"}).as_object().map(|o| o.clone()),
            }
        ])
        .message_id(Uuid::new_v4().to_string())
        .build();
    ...
    let task_id = format!("task_{}", Uuid::new_v4().simple());
    let result = handler.process_message(&task_id, &mixed_message, None).await?;
    println!("Response: {:?}\n", result.status.message);
    ...
    let status_message = Message::builder()
        .role(Role::User)
        .parts(vec![Part::text("What's the status of req_12345?".to_string())])
        .message_id(Uuid::new_v4().to_string())
        .build();

    let task_id = format!("task_{}", Uuid::new_v4().simple());
    let result = handler.process_message(&task_id, &status_message, None).await?;
    println!("Response: {:?}\n", result.status.message);
