...
use crate::{
    ...
    domain::{A2AError, Message, Task, TaskPushNotificationConfig, TaskState, TaskStatusUpdateEvent, TaskArtifactUpdateEvent},
    port::{
...
impl NotificationManager for DefaultBusinessHandler {
    ...
    fn get_task_notification(&self, _task_id: &str) -> Result<TaskPushNotificationConfig, A2AError> {
        Err(A2AError::UnsupportedOperation(
...
impl AsyncMessageHandler for DefaultBusinessHandler {
    async fn process_message<'a>(
        ...
        let message_handler = crate::adapter::business::DefaultMessageHandler::new((*self.storage).clone());
        message_handler.process_message(task_id, message, session_id).await
    }
}
...
impl AsyncTaskManager for DefaultBusinessHandler {
    ...
    async fn update_task_status<'a>(&self, task_id: &'a str, state: TaskState) -> Result<Task, A2AError> {
...
impl AsyncNotificationManager for DefaultBusinessHandler {
    ...
    async fn get_task_notification<'a>(&self, task_id: &'a str) -> Result<TaskPushNotificationConfig, A2AError> {
...
impl AsyncStreamingHandler for DefaultBusinessHandler {
    ...
    async fn broadcast_status_update<'a>(
        ...
        self.storage.broadcast_status_update(task_id, update.status, update.final_).await
    }
    ...
    async fn broadcast_artifact_update<'a>(
        ...
        self.storage.broadcast_artifact_update(
            task_id,
            update.artifact,
            None,
