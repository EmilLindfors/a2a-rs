...
async fn test_basic_streaming_functionality() {
    ...
    .add_skill("stream-test".to_string(), "Stream Test".to_string(), Some("Tests streaming capabilities".to_string()));
    ...
    if let Ok(Ok(mut stream)) = stream_result {
            ...
            while let Some(result) = stream.next().await {
                match result {
                    ...
                    Ok(StreamItem::StatusUpdate(update)) => {
                        ...
                        status_updates += 1;
                        println!("Status update: {:?}, final: {}", update.status.state, update.final_);
        ...
        assert!(!events_received.is_empty(), "Should have received streaming events");
    } else {
...
async fn test_streaming_event_ordering() {
    ...
    .with_streaming()
    .with_state_transition_history()
    .add_skill("ordering-test".to_string(), "Event Ordering Test".to_string(), Some("Tests event ordering".to_string()));
    ...
    if let Ok(Ok(mut stream)) = stream_result {
        ...
        let stream_processing = async {
            while let Some(result) = stream.next().await {
                match result {
                    ...
                    Ok(StreamItem::StatusUpdate(update)) => {
                        event_sequence.push(("StatusUpdate", update.status.state.clone(), update.final_));
        ...
        if !event_sequence.is_empty() {
            ...
            let has_status_updates = event_sequence.iter().any(|(kind, _, _)| kind == &"StatusUpdate");
...
async fn test_streaming_artifact_updates() {
    ...
    .with_streaming()
    .add_skill("artifact-gen".to_string(), "Artifact Generator".to_string(), Some("Generates streaming artifacts".to_string()));
            ...
            while let Some(result) = stream.next().await {
                match result {
                    Ok(StreamItem::ArtifactUpdate(update)) => {
                        ...
                        println!("Artifact update: ID={}, parts={}, last_chunk={}", 
...
async fn test_websocket_connection_resilience() {
    ...
    .with_streaming()
    .add_skill("resilience".to_string(), "Connection Resilience".to_string(), Some("Tests connection handling".to_string()));
    ...
    let invalid_task_result = timeout(
        Duration::from_secs(2),
        client1.subscribe_to_task("", None)
    ).await;
...
async fn test_streaming_with_history_limits() {
    ...
    .with_streaming()
    .with_state_transition_history()
    .add_skill("history-test".to_string(), "History Test".to_string(), Some("Tests history functionality".to_string()));
    ...
    for history_length in history_lengths {
        ...
        if let Ok(Ok(mut stream)) = stream_result {
            ...
            println!("  Failed to establish stream for history length {:?}", history_length);
        }
    }
...
async fn test_streaming_event_specification_compliance() {
    ...
    .with_streaming()
    .with_state_transition_history()
    .add_skill("spec-test".to_string(), "Spec Compliance Test".to_string(), Some("Tests A2A spec compliance".to_string()));
    ...
    if let Ok(Ok(mut stream)) = stream_result {
                ...
                match result {
                    ...
                    Ok(StreamItem::StatusUpdate(update)) => {
                        ...
                        if update.kind != "status-update" {
                            spec_violations.push(format!("Invalid status update kind: {}", update.kind));
                        }
                    ...
                    Ok(StreamItem::ArtifactUpdate(update)) => {
                        ...
                        if update.kind != "artifact-update" {
                            spec_violations.push(format!("Invalid artifact update kind: {}", update.kind));
                        }
                        if update.task_id.is_empty() {
        ...
        assert!(spec_violations.is_empty(), "A2A specification violations detected: {:?}", spec_violations);
    } else {
