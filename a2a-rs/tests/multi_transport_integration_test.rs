...
async fn test_dual_protocol_agent() {
    ...
    .add_skill("echo".to_string(), "Echo Service".to_string(), Some("Returns input as output".to_string()))
    .add_skill("transform".to_string(), "Data Transform".to_string(), Some("Transforms data between formats".to_string()));

    // Start HTTP server
    let http_server = HttpServer::new(processor.clone(), agent_info.clone(), "127.0.0.1:8184".to_string());
    ...
    let ws_server = WebSocketServer::new(processor, agent_info, handler, "127.0.0.1:8185".to_string());
    ...
    let http_agent_card: Value = http_agent_card_response.json().await.expect("Failed to parse HTTP agent card");

    // Verify agent card has dual capabilities
    assert_eq!(http_agent_card["name"].as_str().unwrap(), "Dual Protocol Agent");
    assert!(http_agent_card["capabilities"]["streaming"].as_bool().unwrap());
    assert!(http_agent_card["capabilities"]["pushNotifications"].as_bool().unwrap());
    assert!(http_agent_card["capabilities"]["stateTransitionHistory"].as_bool().unwrap());
    ...
    let http_message = Message::user_text(message_content.to_string(), format!("http-msg-{}", uuid::Uuid::new_v4()));
    let ws_message = Message::user_text(message_content.to_string(), format!("ws-msg-{}", uuid::Uuid::new_v4()));
...
async fn test_cross_protocol_task_interaction() {
    ...
    .with_streaming()
    .with_state_transition_history()
    .add_skill("process".to_string(), "Process Data".to_string(), Some("Processes data with status updates".to_string()));

    // Start servers
    let http_server = HttpServer::new(processor.clone(), agent_info.clone(), "127.0.0.1:8186".to_string());
    let ws_server = WebSocketServer::new(processor, agent_info, handler, "127.0.0.1:8187".to_string());
    ...
    let message = Message::user_text("Process this data".to_string(), format!("msg-{}", uuid::Uuid::new_v4()));
    ...
    let stream_result = timeout(Duration::from_secs(2), ws_client.subscribe_to_task(&task_id, None)).await;
...
async fn test_complex_message_types_across_transports() {
    ...
    .with_streaming()
    .add_skill("analyze".to_string(), "Data Analysis".to_string(), Some("Analyzes various data formats".to_string()));

    // Start servers
    let http_server = HttpServer::new(processor.clone(), agent_info.clone(), "127.0.0.1:8188".to_string());
    let ws_server = WebSocketServer::new(processor, agent_info, handler, "127.0.0.1:8189".to_string());
    ...
    let file_content = b"Sample CSV data\ndate,revenue,users\n2024-01-01,1000,50\n2024-01-02,1100,55";
    ...
    http_message.add_part_validated(file_part.clone()).expect("Failed to add file part to HTTP message");
    ws_message.add_part_validated(file_part).expect("Failed to add file part to WebSocket message");
...
async fn test_error_handling_across_transports() {
    ...
    .with_streaming()
    .add_skill("test".to_string(), "Test Skill".to_string(), Some("For testing purposes".to_string()));

    // Start servers
    let http_server = HttpServer::new(processor.clone(), agent_info.clone(), "127.0.0.1:8190".to_string());
    let ws_server = WebSocketServer::new(processor, agent_info, handler, "127.0.0.1:8191".to_string());
    ...
    let message = Message::user_text("Test message".to_string(), format!("msg-{}", uuid::Uuid::new_v4()));
